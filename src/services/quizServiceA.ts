...
export const quizService = {
  ...
  getQuizzesByCategory: async (categoryId: string): Promise<Quiz[]> => {
    try {
      const response = await api.get<ApiResponse<Quiz[]>>(`/quiz/category/${categoryId}`);
      return response.data?.data || [];
    } catch (error) {
  ...
  submitQuizResult: async (quizId: string, result: {
    ...
    try {
      const response = await api.post<ApiResponse<QuizResult>>(`/quiz/${quizId}/result`, result);
  ...
  getUserResults: async (): Promise<QuizResult[]> => {
    try {
      const response = await api.get<ApiResponse<QuizResult[]>>('/stagiaire/quizzes');
  ...
  getLeaderboard: async (): Promise<any[]> => {
    try {
      const response = await api.get<ApiResponse<any[]>>('/stagiaire/ranking/global');
      return response.data?.data || [];
    } catch (error) {
