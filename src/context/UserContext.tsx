...
export function UserProvider({ children }: { children: ReactNode }) {
  const VITE_API_URL =
    import.meta.env.VITE_API_URL || "http://127.0.0.1:8000api";
  ...
  const refetchUser = async () => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:8000api/me", {
        headers: {
    ...
    if (file && user) {
      ...
      try {
        const response = await fetch(
          `http://127.0.0.1:8000api/avatar/${user.id}/update-profile`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${token}`,
