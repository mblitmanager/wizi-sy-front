...
import FormateurService, { TrainerPerformanceResponse, StudentDetails, FormationPerformance, StagiaireFormationPerformance, StudentPerformance } from '@/services/FormateurService';
                ...
                {loading ? (
                    ...
                    <div className="p-10 space-y-10 max-h-[75vh] overflow-y-auto custom-scrollbar">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {[
                                { label: 'Quiz Totaux', value: details.quiz_stats?.total_quizzes || details.quiz_stats?.total_quiz || 0, icon: <Trophy className="h-4 w-4" />, color: 'brand-primary' },
                                { label: 'Score Moyen', value: `${details.quiz_stats?.avg_score || 0}%`, icon: <CheckCircle2 className="h-4 w-4" />, color: 'green' },
                                { label: 'Précision', value: `${details.quiz_stats?.total_correct || 0}/${details.quiz_stats?.total_questions || 0}`, icon: <Smartphone className="h-4 w-4" />, color: 'blue' },
                                { label: 'Record personnel', value: `${details.quiz_stats?.best_score || 0}%`, icon: <Trophy className="h-4 w-4" />, color: 'orange' }
                            ].map((stat, i) => (
                        ...
                        <div className="space-y-6 pt-6 border-t border-slate-100">
                            ...
                            <div className="grid gap-4">
                                {[
                                    { label: 'Dernier signal', value: details.activity?.last_activity || 'Inconnu', icon: <Clock className="h-4 w-4" /> },
...
export const RankingsSection = ({ data }: { data: TrainerPerformanceResponse }) => {
    return (
...
const TrainerPerformanceStats = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const perfRes = await FormateurService.getStudentsPerformance();
                setData(perfRes);
            } catch (err) {
