...
function getAdContent(formation) {
  // Génère dynamiquement un titre, une description, un bénéfice et un CTA
  const titre = formation.titre || formation.title || "Formation";
  const desc = stripHtml(formation.description)?.slice(0, 120) || "Une formation incontournable pour progresser rapidement.";
  const titles = [
  ...
  // Sélectionne aléatoirement un titre, description, bénéfice, cta
  return {
...
const AdCatalogueBlock = ({ formations }) => {
  if (!formations || formations.length === 0) return null;
  const selected = getRandomItems(formations, 3);
  return (
    <div className="grid gap-6 md:grid-cols-3 sm:grid-cols-1 my-8">
      {selected.map((formation, idx) => {
        const ad = getAdContent(formation);
        return (
          <div
