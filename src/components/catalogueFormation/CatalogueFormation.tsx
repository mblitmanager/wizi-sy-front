...
export default function CatalogueFormation({
  ...
  // Safely handle various formations data structures
  const formations = catalogueData?.stagiaire?.formations || [];
    ...
    <div className="mb-8 p-4 bg-background rounded-lg shadow">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {formations.map((formation: Formation) => {
          // Handle both array and object formats for catalogue_formation
          const catalogueItems = Array.isArray(formation.catalogue_formation)
            ? formation.catalogue_formation
            : formation.catalogue_formation
            ? [formation.catalogue_formation]
            : [];

          return catalogueItems.map((catalogue) => {
